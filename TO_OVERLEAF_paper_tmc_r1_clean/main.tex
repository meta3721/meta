...
\begin{itemize}
    ...
    correction uses only legal pre-outcome information, combines local
    H\'ajek normalization with usable-update correction, and yields a
    selection-corrected reference mass for aggregation.  Under explicit support,
    ...
    Debt remains an internal accounting term, not a separate contribution.
\end{itemize}
...
R/O/U legal histories, and finite-support calibration.

We evaluate these contributions with a no-harm control, a risk-semantic
block, SensorScope and U-Air reconstruction experiments, and a T-Drive
opportunity replay.  RAVEN-MCS uses support-conditional Design: on when
...
federated completion, and participation-aware FL.
...
\begin{itemize}
    ...
    deployment-target risk mismatch, including under real mobility
    opportunity?
    ...
    \item \textbf{RQ3:} How do the core correction and calibration components
    contribute, and when is Design useful as a reconstruction factor?
\end{itemize}
...
opportunity.  E3 addresses RQ2 on the two scientifically comparable
SensorScope and U-Air blocks, using support-conditional Design
(Design on SensorScope; Design off on U-Air).  E4 addresses RQ3 by
...
provide the main E3/E4 evidence and the coverage-stress audit.  For both, the deployment-target mapping
...
cell $\times$ time block $\times$ day type & -- & G3 replay, not ranking\\
\bottomrule
\end{tabular}
\end{table*}
...
The main E3/E4 condition is \emph{Complete-aligned}, in which opportunity,
observation, and usable-update selection are all active and non-random in the
same direction.  All compared methods within a paired block consume the same
...
Obs has weaker endpoint-specific effects, Use
has no clear isolated effect in this family, Inst trades finite-window
structure against prediction, and Debt is statistically consistent but
...
mismatch without supplying a method ranking.  Usable-update correction
has no clear isolated effect in the tested family, and target debt is
quantitatively small.

These guarantees hold under explicit support and sequential
identification.  They do not cover arbitrary hidden confounding,
misspecified targets, or end-to-end mobile systems.  Extending the
replay to jointly measured observation and network processes remains future work.
...
and cited in Section~\ref{subsec:eval_setup}.  Frozen selection traces,
target maps, the $20$ paired seeds, and the tabulated results in this
paper are archived with the submission.  FedAU-Window and ObsUse-Window
are windowed adaptations, not official reproductions of those systems.
A complete public snapshot of the sealed training stack is not claimed
at submission.
