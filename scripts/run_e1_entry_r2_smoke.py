#!/usr/bin/env python3
"""Run the frozen one-seed five-method E1-R2 official-entry smoke."""
...
def main() -> int:
    ...
    if set(runs) != expected:
        raise RuntimeError(f"R2 smoke method mismatch: {sorted(runs)}")
    ...
    fa_pred = pd.read_parquet(
        runs["fedasync_window"] / "predictions_test.parquet",
    )["y_pred"].to_numpy()
    ta_pred = pd.read_parquet(
        runs["flamf_timealign_adapted"] / "predictions_test.parquet",
    )["y_pred"].to_numpy()
    comparison = {
        "max_l1_alpha_difference": float(np.nanmax(alpha_l1)),
        "prediction_max_absolute_difference": float(
            np.max(np.abs(fa_pred - ta_pred)),
        ),
        "fedasync_final_model_hash": json.loads(
            (runs["fedasync_window"] / "manifest.json").read_text(),
        )["model_hash"],
        "timealign_final_model_hash": json.loads(
            (
                runs["flamf_timealign_adapted"] / "manifest.json"
            ).read_text(),
        )["model_hash"],
    }
    if comparison["max_l1_alpha_difference"] <= 1e-10:
        raise RuntimeError("official TimeAlign does not differ from FedAsync")
    (root / "timealign_fedasync_comparison.json").write_text(
        json.dumps(comparison, indent=2), encoding="utf-8",
    )
    print(f"Official E1-R2 entry smoke complete: {root}")
    return 0
