#!/usr/bin/env python3
"""Seed-major scheduler for sealed E1 formal 25-run execution."""
...
from raven_mcs.experiments.e1_entry import (  # noqa: E402
    E1_METHODS,
    E1_SEEDS,
    run_official_method,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.dry_run_scheduler:
        if methods != list(E1_METHODS):
            raise ValueError("formal execution requires frozen method order")
        if seeds != list(E1_SEEDS):
            raise ValueError("formal execution requires frozen seeds 26001-26005")
        if args.windows != 100:
    ...
    if protocol.get("authorization_status") != "AUTHORIZED_FOR_FROZEN_EXECUTION":
        raise RuntimeError("frozen protocol is not authorized for formal execution")
    ...
    for index, item in enumerate(matrix):
        ...
        progress["current_window"] = args.windows
    ...
    progress["formal_performance_result"] = (
        not progress["failed"] and len(progress["completed"]) == 25
    )
    ...
    return 0 if progress["formal_performance_result"] else 1
