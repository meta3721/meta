#!/usr/bin/env python3
"""Append one FREEZE-R1.1 stage to exact-command JSONL."""
...
def main() -> int:
    ...
    parser.add_argument("--historical-or-current", default="current")
    ...
    if command[0] in {"python", "python3", "py"}:
        command[0] = str(venv_python) if venv_python.exists() else sys.executable
    logs = root / "logs"
    ...
    stdout = logs / f"E1_FORMAL_FREEZE_R1_1_{args.stage.upper()}_STDOUT.log"
    stderr = logs / f"E1_FORMAL_FREEZE_R1_1_{args.stage.upper()}_STDERR.log"
    ...
    row = {
        ...
        "historical_or_current": args.historical_or_current,
    }
